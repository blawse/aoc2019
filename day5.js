...
function execute(input, IN_VALUE) {
   ...
   while (true) {
      let opcode = output[p] % 100;
      let mode1 = Math.floor(output[p]/100) % 10;
      let mode2 = Math.floor(output[p]/1000) % 10;

      if (opcode === OP_ADD) {
         let v1 = (mode1 ? output[p+1]:output[output[p+1]]);
         let v2 = (mode2 ? output[p+2]:output[output[p+2]]);
...
let result1 = execute(input, 1);
console.log("Result for Day 5, Part 1:", result1);

let result2 = execute(input, 5);
console.log("Result for Day 5, Part 2:", result2);
