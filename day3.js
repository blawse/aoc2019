...
function mapWire(path, intersections, findMinimum, findMinimumSteps) {
   ...
   paths.forEach((p) => {
      ...
      for (let i=0; i<steps; i++) {
         ...
         wire[position] = totalSteps;

         if (intersections  &&  intersections[position]) {
            let stepsToThisIntersection = intersections[position] + totalSteps;
            if (stepsToThisIntersection < minSteps) minSteps = stepsToThisIntersection;

            let distance = Math.abs(r) + Math.abs(c);
            if (distance < minDistance) minDistance = distance;
            intersectsAt[position] = distance;
         }
      }
   });

   if (findMinimumSteps) return minSteps;
   if (findMinimum) return minDistance;
...
let wire1 = mapWire(w1path);
let minDistance = mapWire(w2path, wire1, true);
let minSteps = mapWire(w2path, wire1, false, true);

console.log("Result for Day 3, Part 1:", minDistance);
console.log("Result for Day 3, Part 2:", minSteps);